...
def main():
    ...
    while True:
        ...
        # Check if ChatGPT can answer the query with the given context
        can_answer = decision_agent(query, context)

        if not can_answer:
            print("Not finding an answer in the class docs, searching Google...")
...
def main_final():
    ...
    while True:
        ...
        if not did_answer:
            # Stop the default spinner and start the Google search spinner
            stop_spinner()
            start_spinner("Searching Google...")
